...
export async function POST(req: Request) {
  ...
  try {
    ...
    // Create the vote without checking for existing votes
    const vote = await prisma.vote.create({
      data: {
        userId: dbUser.id,
        profilePictureId: selectedUser.profilePicture[0].id,
        value: 1,
      },
    });
    ...
    console.error('Error casting vote:', error); // Log the full error object
    return NextResponse.json({ error: 'Error casting vote' }, { status: 500 });
  }
}
